...
export function messageSendV1 (token: string, channelId: number, message: string): MessageSendV1 {
  ...
  // Gary: Use .find() or helper functions to get the channel or dm and validate the user
  ...
  // Gary: Error checks should be at the top of the file
...
export function messageEditV1 (token: string, messageId: number, message: string): MessageEditV1 {
  ...
  for (const channel of data.channels) {
    ...
    for (const msg of channel.messages) {
      ...
      // Gary: Would the message ever be null or undefined?
