...
function channelInviteV1(authUserId: number, channelId: number, uId: number): ChannelInviteV1 {
  ...
  // Gary: Check if user exists first, and then assign variables
...
function channelMessagesV1(authUserId: number, channelId: number, start: number): ChannelMessagesV1 {
  const data = getData();
  // Gary: Use booleans instead of numbers for flags, otherwise we don't know what each numerical value means
  let existChannel = 0;
  let existAuth = 0;
  ...
  // Gary: Error checks should be at the top
...
// Gary: Missing docstring (applies for all functions below as well)
//       Also perhaps remove `export` and export function at the bottom to keep things consistent
export function channelAddOwnerV1(token: string, channelId: number, uId: number) {
  // Gary: `const { channels } = getData();`
  const data = getData();
