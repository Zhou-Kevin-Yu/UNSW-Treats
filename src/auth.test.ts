...
describe('Testing valid returns from register user', () => {
  test('3 registered users', () => {
    // Gary: Checking that the authUserId is some number is not blackbox
    //       expect(returned).toStrictEqual(expect.objectContaining({ authUserId: expect.any(Number) }))
    //       ^Code above ensures that it returns an object with just 1 key and that the value is any number
